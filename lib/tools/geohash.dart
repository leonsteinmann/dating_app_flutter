...
// Code from: https://github.com/nfrawley93/dart_geohash
// Adaptions are made for null-safety support
...
class GeoHasher {
  ...
  double _bitsToDouble({required List<int> bits, double lower = -90.0, double middle = 0.0, double upper = 90.0}) {
    bits.forEach((bit) {
      if (bit == 1) {
        lower = middle;
      } else {
  ...
  List<int> _doubleToBits({required double value, double lower = -90.0, double middle = 0.0, double upper = 90.0, int length = 15}) {
    List<int> ret = <int>[];
  ...
  String _bitsToGeoHash(List<int> bitValue) {
    ...
    List<String> geoHashList = <String>[];
